...
def get_label_data(excel_path, img_path):
    ...
    df_id0_label['realpath'] = df_id0_label['ID'].apply(lambda x: os.path.join(img_path, str(x), df_path['path'][0]))
    for i in range(1, 100):    # 400*100 = 40,000개 path 생성 - 마지막은 해당되는 만큼
...
plt.savefig('./models/weights/batch40000/gender_train_weights1.png')
...
# 가중치 저장
model.save_weights("./models/weights/batch40000/gender_train_weights1.h5") # 불러다 evaluate(테스트셋 성능 평가)은 가능하나, 이 아이로 이어서 추가학습을 시킬 수 있는 건 아닌 것 같음
